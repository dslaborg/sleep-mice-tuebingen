...
class DataViewer:
    ...
    def run(self):
        ...
        _, self.axes = plt.subplots(len(self.view_config.CHANNELS) + 2, sharex='all')
        plt.subplots_adjust(bottom=0.18, top=0.95, hspace=0.1)
        ...
        slider_sample = Slider(ax_sample, 'sample', 1, len(self.datamap_view) - 12, valinit=0, valstep=1, valfmt='%i')
        ...
        self.lines['scores'], = self.axes[-2].plot(t, np.array(init_data[1]).repeat(self.window_points), c='black',
                                                   lw=3)
        self.axes[-2].set_ylim((-0.1, len(self.view_config.STAGES) - 0.5))
        self.axes[-2].set(yticks=np.arange(len(self.view_config.STAGES)), yticklabels=self.view_config.STAGES)
        self.axes[-2].set(ylabel='labels')
        self.axes[-2].grid()
    ...
    def update_axes(self):
        ...
        scores = np.array([data[i][1] for i in range(self.window_size)]).repeat(self.window_points)
    ...
    def update_prediction_axis(self):
        ...
        self.axes[-1].set(xlabel='epoch', ylabel='probabilities')
        ...
        self.axes[-1].legend(handles, self.view_config.STAGES, loc=(1.01, 0.0))
